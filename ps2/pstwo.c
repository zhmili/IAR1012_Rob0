...
1、手柄解码，识别按键值，读取模拟值。       
**********************************************************/	 
#define DELAY_TIME  DelayNus(5); 
#define delay_us  DelayNus
...
//发送模式设置
void ps2_cfg(int ana_dig, int mode_lock)
{
  CS_L;
	PS2_Cmd(0x01);  
	PS2_Cmd(0x44);  
	PS2_Cmd(0X00);
	PS2_Cmd(ana_dig);   //analog=0x01;digital=0x00  软件设置发送模式
	PS2_Cmd(mode_lock); //Ox03锁存设置，即不可通过按键“MODE”设置模式。
