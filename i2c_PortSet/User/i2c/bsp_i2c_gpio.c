...
uint8_t i2c_WaitAck(int port)
{
	...
	macI2C_SDA_1(port);	/* CPU释放SDA总线 */
	i2c_Delay();
	macI2C_SCL_1(port);	/* CPU驱动SCL = 1, 此时器件会返回ACK应答 */
	i2c_Delay();
...
{
	macI2C_SDA_0(port);	/* CPU驱动SDA = 0 */
	i2c_Delay();
	macI2C_SCL_1(port);	/* CPU产生1个时钟 */
	i2c_Delay();
	macI2C_SCL_0(port);
	i2c_Delay();
	macI2C_SDA_1(port);	/* CPU释放SDA总线 */
...
uint8_t i2c_CheckDevice(uint8_t _Address, int port)
{
	...
	i2c_Stop(port);
//	i2c_CfgGpio();		/* 配置GPIO */
	if (macI2C_SDA_READ(port) && macI2C_SCL_READ(port))
	{
	i2c_Start(port);		/* 发送启动信号 */
	
	/* 发送设备地址+读写控制bit（0 = w， 1 = r) bit7 先传 */
	i2c_SendByte(_Address | macI2C_WR,port);
	ucAck = i2c_WaitAck(port);	/* 检测设备的ACK应答 */

	i2c_Stop(port);			/* 发送停止信号 */
