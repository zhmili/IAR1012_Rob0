...
//  GPIO_ResetBits(GPIOE, GPIO_Pin_2);
 ...
 // j= GPIO_ReadInputDataBit(GPIOA, DEF_BIT_06); 
 // j= GPIO_ReadInputDataBit(GPIOA, DEF_BIT_07);
 // j= GPIO_ReadInputDataBit(GPIOB, DEF_BIT_06);
 // j= GPIO_ReadInputDataBit(GPIOB, DEF_BIT_07);
    
 // Alternate Function 功能不能用输出用 受别的控制
 // GPIO_SetBits(GPIOA, DEF_BIT_06);
 // GPIO_SetBits(GPIOA, DEF_BIT_07);
 // GPIO_SetBits(GPIOB, DEF_BIT_06);
 // GPIO_SetBits(GPIOB, DEF_BIT_07);
...
void RxTxISR_Handler (u8 port)
{
  ...
  if(USART_GetITStatus(USARTx, USART_IT_RXNE) != RESET)
  {
    ...
    if(RecvBuf[port].buf[RecvBuf[port].act_lenth] == '5')
    {
